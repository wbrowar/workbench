...
module.exports = {
  configureWebpack: config => {
    config.resolve = {
      alias: {
        '~': path.resolve(wb.paths.wb.src),
        Components: path.resolve(wb.paths.components.src),
        CSS: path.resolve(wb.paths.css.src),
        GQL: path.resolve(`${wb.paths.wb.src}gql/`),
        JS: path.resolve(wb.paths.js.src),
        Source: path.resolve(wb.paths.wb.source),
        WB: path.resolve(wb.paths.wb.workbench),
        Views: path.resolve(`${wb.paths.wb.src}views/`),
      },
    }
  },
  chainWebpack: config => {
