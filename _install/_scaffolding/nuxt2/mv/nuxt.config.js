...
export default {
  build: {
    ...
    extend(config, ctx) {
      ...
      config.resolve.alias.GQL = path.resolve(`${wb.paths.wb.src}gql/`);
      config.resolve.alias.JS = path.resolve(wb.paths.js.src);
      config.resolve.alias.Layouts = path.resolve(`${wb.paths.wb.src}layouts/`);
      config.resolve.alias.Pages = path.resolve(`${wb.paths.wb.src}pages/`);
      config.resolve.alias.Source = path.resolve(wb.paths.wb.source);
      config.resolve.alias.WB = path.resolve(wb.paths.wb.workbench);
      config.resolve.alias.Templates = path.resolve(`${wb.paths.wb.src}templates/`);
    },
  },
      ...
      if (wb.enableDocs) {
        ...
        componentDocPages.forEach((item) => {
          ...
          routes.push({
            ...
            component: `${wb.paths.wb.workbench}docs/vue/ComponentDocs.vue`,
            params: {
              slug,
            },
          });
