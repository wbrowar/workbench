<template>
  <div>
    ...
    <CodeExample :code="code.default" title="Share Button" description="When clicked, a share sheet appears on browsers that support the Share API. Hides itself on browsers that do not support the Share API.">
      <Share share-title="This is Header" share-text="This is the body text." :share-url="linkToShare" />
      <p><em>NOTE: the share button will only appear if your browser supports the <a href="https://developer.mozilla.org/en-US/docs/Web/API/Navigator/share">Share API</a>.</em></p>
    </CodeExample>

    <CodeExample :code="code.full" title="Share + Fallback" description="Add fallback content for browsers that do not support the Share API.">
  ...
  methods: {
    copyToClipboard: function (text) {
      navigator.clipboard.writeText(text).then(() => {
        ...
        this.$refs.messenger.addMessage({ closeButton: true, level: 'success', messageHtml: `<p>"${this.linkToShare}" copied to clipboard.</p>` });
      }, () => {
        log(`Could not copy ${ text } to clipboard`);
        this.$refs.messenger.addMessage({ closeButton: true, level: 'error', messageHtml: `<p>Copying "${this.linkToShare}" to clipboard failed. Please try again.</p>` });
      });
    },
...
<Messenger ref="messenger" docked :message-duration="5000" />`,
    ...
    this.events = [
      { name: 'shareSuccess', description: `Fires when the share action is complete.` },
      { name: 'shareError', arguments: 'String (error message)', description: `Fires when an error occurs or when the user decides to decline sharing even when a sharing action has begun.` },
    ];
    this.modifiers = [
    ...
    this.props = [
      ...
      { name: 'shareText', type: 'String', description: `Describes the URL to be shared. Can be used as body text in some sharing actions.` },
