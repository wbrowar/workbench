<template>
  <div>
    ...
    <CodeExample dark :code="code.changeColors" title="Color Replacements" description="Change colors of <svg> elements. NOTE: This does not work with background or image SVG icons.">
      <div style="width: 40px;">
    ...
    <CodeExample :code="code.background" title="Content Image" description="Creates and image with the SVG set to its src attribute.">
      <div style="width: 40px;">
...
<IconSVG handle="plus" :replacements="{ '<polygon': \`<polygon fill='yellow'\` }" />`,
    ...
    this.props = [
      ...
      { name: 'colorVar', type: 'String', description: `Style <svg> elements with a color variable as defined in 'package.json'.` },
      { name: 'handle', type: 'String', description: `The name of the svg based on its original filename. For example, an SVG at '_source/_icon/plus.svg' would have the handle 'plus'` },
      { name: 'image', type: 'Boolean', description: `Sets the SVG as a src attribute in an <img> element.` },
      { name: 'replacements', type: 'Object', description: `Perform string replacements on using key/value paring. This can be used to set fill colors, replace titles, or remove elements from the SVG code.` },
    ]
  },
};
</script>